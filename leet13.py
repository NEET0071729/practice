...
def romanToInt(s):
    ...
    for i in range(1, len(tup) + 1):
        if count == 1:                                                                   #To skip the index for else statement
            count = 0
            continue
        elif i == len(tup):
            number += romanDict[tup[i * (-1)]]
            break
        elif romanDict[tup[i * (-1)]] <= romanDict[tup[i*(-1) - 1]]:
            number += romanDict[tup[i * (-1)]]
        else:
            number = number + romanDict[tup[i * (-1)]] - romanDict[tup[i*(-1) - 1]]
            count = 1
    return number
